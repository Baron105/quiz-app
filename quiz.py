...
def fetch_questions(amount=10, category=9, difficulty='easy', question_type='multiple'):
...
class StartWindow:
    ...
    def start_gui(self):
        ...
        self.start_window.geometry("300x200")
        self.start_window.title("Quiz Start")

        category_label = Label(self.start_window, text="Select Category:")
        category_label.pack()

        self.category_var = StringVar(self.start_window)
        self.category_var.set(category_options[0])
        category_dropdown = OptionMenu(self.start_window, self.category_var, *category_options)
        category_dropdown.pack()

        difficulty_label = Label(self.start_window, text="Select Difficulty:")
        difficulty_label.pack()

        self.difficulty_var = StringVar(self.start_window)
        self.difficulty_var.set(difficulty_options[0])
        difficulty_dropdown = OptionMenu(self.start_window, self.difficulty_var, *difficulty_options)
        difficulty_dropdown.pack()

        amount_label = Label(self.start_window, text="Enter Number of Questions: (Preferably < 20)")
        amount_label.pack()

        self.amount_entry = Entry(self.start_window)
    ...
    def start_quiz(self):
        category = category_options.index(self.category_var.get()) + 9
        difficulty = self.difficulty_var.get()
        amount = int(self.amount_entry.get())

        fetch_questions(amount=amount, category=category, difficulty=difficulty.lower(), question_type='multiple')
